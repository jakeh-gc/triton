...
def _fwd_kernel(Q, K, V, sm_scale, Out, stride_qm, stride_qk, stride_kn, stride_kk, stride_vn, stride_vk, stride_om,
    ...
    m_i_final = tl.zeros([BLOCK_M], dtype=tl.float32) - float("inf")
    l_i_final = tl.zeros([BLOCK_M], dtype=tl.float32)
    for i in range(num_warps):
        mask = (offs_b[:, None] == i)
        mask = mask.to(tl.int32)
        msi = tl.sum(m_i * mask, axis=0)
        lsi = tl.sum(l_i * mask, axis=0)
        mask1 = (offs_b[:, None, None] == i)
        mask1 = mask1.to(tl.int32)
        osi = tl.sum(acc * mask1, axis=0)
        alphai = tl.math.exp2(m_i_final - msi)
        l_i_final = l_i_final * alphai + lsi
        o2 = o2 * alphai[:, None] + osi
        m_i_final = msi
    o2 = o2 / l_i_final[:, None]
...
def test_op():
    ...
    print(f"torch_output={torch_out1}")

    # assert torch.allclose(torch_out2.to(o.dtype), o, atol=1e-2, rtol=0)
    assert torch.allclose(torch_out1.to(o.dtype), o, atol=1e-2, rtol=0)
